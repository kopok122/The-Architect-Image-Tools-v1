...
# ==============================================
# merged_tools_with_tabs.py
# Unified tabbed GUI wrapping three original tools
# ==============================================
...
class AdvancedExifToolGUI(tk.Frame):
    def __init__(self, parent=None):
        super().__init__(parent if parent is not None else parent if parent is not None else tk.Tk())
        self.pack(fill='both', expand=True)
        self.master.title("Advanced Universal Comment Editor")
...
if __name__ == "__main__":
    try:
        ...
        if app.winfo_exists(): app# mainloop controlled by UnifiedApp
    except Exception as e:
...
class AdvancedFileRandomizer:
    ...
    def run(self):
        if len(sys.argv) > 1 and os.path.isdir(sys.argv[1]):
            self.folder_path.set(sys.argv[1])
            self.load_items_to_preview()
        # mainloop controlled by UnifiedApp
...
class UnifiedApp(tk.Tk):
    def __init__(self):
        ...
        tools_menu.add_command(label="OCR Processor", command=lambda: self.open_tab('OCR'))
        tools_menu.add_command(label="ExifTool Editor", command=lambda: self.open_tab('Exif'))
        tools_menu.add_command(label="File Randomizer", command=lambda: self.open_tab('Randomizer'))
        ...
        tk.Button(toolbar, text="OCR", command=lambda: self.open_tab('OCR')).pack(side="left", padx=4, pady=2)
        tk.Button(toolbar, text="ExifTool", command=lambda: self.open_tab('Exif')).pack(side="left", padx=4, pady=2)
        tk.Button(toolbar, text="Randomizer", command=lambda: self.open_tab('Randomizer')).pack(side="left", padx=4, pady=2)

        # --- Notebook Tabs ---
        self.notebook = ttk.Notebook(self)
        self.notebook.pack(fill="both", expand=True)

        # Keep track of tabs by key
        self._tabs = {}

        # Welcome tab
        welcome = tk.Frame(self.notebook)
        tk.Label(welcome, text="✨ Welcome to Unified Tool Suite ✨", font=("Arial", 16)).pack(pady=20)
        tk.Label(welcome, text="Gunakan menu atau tombol di atas untuk membuka tool.", font=("Arial", 12)).pack(pady=10)
        self.notebook.add(welcome, text="Welcome")
        self._tabs['Welcome'] = welcome

    def open_tab(self, tool_key):
        # Prevent duplicate tabs: if exists, select it
        key_map = {'OCR':'OCR Processor', 'Exif':'ExifTool Editor', 'Randomizer':'File Randomizer'}
        if tool_key not in key_map:
            return
        tab_name = key_map[tool_key]
        # If already open, select and return
        if tab_name in self._tabs:
            self.notebook.select(self._tabs[tab_name])
            return
        ...
        try:
            if tool_key == 'OCR':
                ...
                AdvancedExifToolGUI(frame)
            elif tool_key == 'Randomizer':
                AdvancedFileRandomizer(frame)
        except Exception as e:
            # Show error in tab
            tk.Label(frame, text=f"Failed to load {tab_name}: {e}").pack(padx=10, pady=10)
        self.notebook.add(frame, text=tab_name)
        self._tabs[tab_name] = frame
        self.notebook.select(frame)

    def show_about(self):
        messagebox.showinfo("About", "Unified Tool Suite\\nMade with ❤️ by Faizal & Nyra")
